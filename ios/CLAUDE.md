...
**Xcode 27 needs Tuist 4.208.0 or later — the version `.mise.toml` pins.** Xcode 27 rejects
any deployment target below iOS 15.0, and several Google/Firebase packages (GoogleUtilities,
Promises, leveldb, nanopb, the Mobile Ads and UMP wrappers) plus Lottie and WaterfallGrid
still declare iOS 12 or 13. From 4.197.1 on, Tuist raises every generated package target to
at least 15.0 on the Swift 6.4 toolchain, resource-bundle targets included, so no
post-generation patching is needed. If a build fails with "range of supported deployment target
versions is 15.0 to 27.0.x", the generated projects on disk came from an older Tuist: check
that `tuist version` prints the pinned version, then `rm -rf Tuist/.build/tuist-derived` and
rerun `tuist install` and `tuist generate --no-open`.
