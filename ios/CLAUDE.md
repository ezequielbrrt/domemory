...
Google/Firebase SPM package projects under `Tuist/.build/tuist-derived/Projects/` (leveldb,
GoogleUtilities, Promises, nanopb, GoogleDataTransport, GoogleMobileAds,
GoogleUserMessagingPlatform, GoogleAdsOnDeviceConversion, Lottie, WaterfallGrid) declare an
internal deployment target as low as 12.0, below the iOS 27 Simulator SDK's 15.0 floor, so a
plain build fails immediately with "range of supported deployment target versions is 15.0 to
27.0.x." `Tuist/Package.swift`'s `PackageSettings.baseSettings` alone does **not** fix this —
it only reaches each generated project's project-level default, not the explicit
`IPHONEOS_DEPLOYMENT_TARGET` Tuist writes directly onto most individual targets (which wins),
nor the resource-bundle targets Tuist auto-synthesizes for a package's `resources:`
declaration (not addressable through `PackageSettings` by any name at all). The script
normalizes every generated target uniformly to 18.6, this app's real minimum — uniformly,
because raising only the targets that were failing outright just moves the failure: Swift's
cross-module check then refuses a target still sitting at a lower value (e.g. Firebase's own
targets, valid at 15.0) importing a dependency now raised to 18.6. A scheme pre-action was
tried first and doesn't work — xcbuild validates deployment targets before any pre-action
script runs, even within the same build, so patching from inside the build is always too
late; the script only works because it patches the files on disk beforehand. If you ever see
the "range of supported deployment target versions" error again, you skipped this step, not
a regression.

`project.pbxproj` is generated by Tuist and checked in. After adding, renaming, or deleting a source file, run `tuist generate --no-open` from `DoMemory/` so the target picks it up (then rerun the fix script); run `tuist install` first if the SPM tree is stale. Dependencies are declared in `Project.swift` (target) and `Tuist/Package.swift` (versions).
...
- **Single-column screens** cap themselves with `.readableWidth(_:)` from `AppConfiguration.swift`. Lists and forms (Settings, Achievements) use the default `Layout.readableWidth` (640pt); narrower stacks of buttons and controls (the multiplayer lobby, the finished-match actions, the intro carousels' buttons) pass `Layout.modalWidth` (460pt). **Centred cards over a board** (Win, Lose, Pause, Quit, Out of lives) cap at `Layout.modalWidth` with `.frame(maxWidth:)`. Every iPhone is narrower than both, so neither changes a phone. Give any new full-screen view the same treatment.
